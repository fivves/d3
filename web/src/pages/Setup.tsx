...
export function Setup() {
  ...
  async function finish() {
    ...
    try {
      const { data } = await api.post('/auth/signup', form, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  return (
    <div className="container" style={{ maxWidth: 720, paddingTop: 40 }}>
      ...
      {step === 1 && (
        <div className="card">
          <div className="heading">Choose a username</div>
          <label>Username</label>
          <input value={username} onChange={(e)=>setUsername(e.target.value)} placeholder="eddie" />
          <div className="sub">3–30 characters, letters/numbers/underscore. Cannot be changed later.</div>
          <div style={{ display:'flex', gap:8, marginTop: 12 }}>
            <button className="button secondary" onClick={back}>Back</button>
            <button className="button" onClick={next} disabled={!/^[a-zA-Z0-9_]{3,30}$/.test(username)}>Next</button>
          </div>
        </div>
      )}
      {step === 2 && (
        <div className="card">
          <div className="heading">Set a 4‑digit PIN</div>
          <label>PIN</label>
